...
#define MEASURE_ON()   \
	do { \
		...
		stm32_gpioPinWrite(((struct stm32_gpio *)GPIOB_BASE), MEASURE_SWITCH_MASTER, 1); \
	} while(0)

#define MEASURE_OFF()  \
	do { \
		stm32_gpioPinWrite(((struct stm32_gpio *)GPIOC_BASE), MEASURE_SWITCH, 1); \
		stm32_gpioPinWrite(((struct stm32_gpio *)GPIOB_BASE), MEASURE_SWITCH_MASTER, 0); \
	} while(0)
...
#define MEASURE_INIT() \
	do { \
		...
		stm32_gpioPinConfig(((struct stm32_gpio *)GPIOB_BASE), MEASURE_SWITCH_MASTER, GPIO_MODE_OUT_PP, GPIO_SPEED_50MHZ);\
	} while(0)
